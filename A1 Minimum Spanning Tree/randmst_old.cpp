...
//Structs for a point in 2D, 3D, and 4D space. We could have made this more
//general by allowing any number of dimensions, but 
struct Point2D {
	double x, y;
};
 ...
 */
struct MinimumSpanningTree prim(int numPoints, vector<struct Edge> adjLists[numPoints]) {
 ...
 */
struct MinimumSpanningTree kruskal(int numPoints, vector<struct Edge> adjLists[numPoints], vector<struct Edge> edges) {
...
int main(int argc, char *argv[]) {
	...
	vector<struct Edge> adjLists[numPoints];
	...
	if (debug == DEBUG) {
		...
		edges.push_back(e11);
		edges.push_back(e12);
	} else if (dimension == 0) { //Edge weights ~ Uniform(0, 1)
		for (int i1 = 0; i1 < numPoints; i1++) {
		...
		//for (int i1 = 0; i1 < numPoints; i1++) {
		//	cout << i1 << ". " << points[i1][0] << " " << points[i1][1] << endl;
		//}
		...
		//for (int i1 = 0; i1 < edges.size(); i1++) {
		//	cout << edges[i1].v1 << " " << edges[i1].v2 << " " << edges[i1].weight << endl;
		//}
